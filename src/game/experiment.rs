...
use glium::{glutin::{self, event}, Surface};
...
// SPHERE
    ...
    let mut sphere = Mesh::new(translation , &app.screen , &sphere_data.verts , sphere_shaders);
...
// TERRAIN OBJECT
    ...
    let mut terrain = Mesh::new(t , &app.screen , &terrain_data.verts , terrain_shader);


    let mut _camera = Camera::new(&app.screen);
    _camera.update();

    let mut children:Vec<Mesh> = vec![sphere, terrain];

    let mut w = world::World::new(children, _camera, (1., 1., 1.));
